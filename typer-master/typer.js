			...
			.css({
				...
				for(var i = 0;i < words.length;i++) {
					...
					if(string.toLowerCase().indexOf(typed_string.toLowerCase()) == 0) {
						...
						if(word.get('highlight')!=0)
							score-=10;//score minus jika salah , soal no 7
...
var Typer = Backbone.Model.extend({
	...
	start: function() {
		var animation_delay = 40;//soal no 2
		var self = this;
	...
	iterate: function() {
		...
		for(var i = 0;i < words.length;i++) {
			...
			{
				score+=100;//score plus jika benar, soal no 6
			}	
